...
### UCRL2
    ...
    for m in range(num_runs):
        ...
        for k in range(num_episodes):
            ...
            regret_ucrl[m,k] = abs(-computed_solution[0][0]-true_solution[0][0])
...
if __name__ == "__main__":   
    ...
    for k in range(num_episodes):
        for m in range(num_runs):
            ...
            # Compute current solution
            cur_solution = sampled_mdp.rsolve_mpi(b"robust_l1",np.array(thresholds))
            cur_policy = cur_solution.policy
            action = cur_policy[0] # action for the nonterminal state 0
            regret_bayes_ucrl[k,m] = abs(cur_solution[0][0]-true_solution[0][0])
