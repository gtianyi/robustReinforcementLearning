...
# 3 possible actions, each action can lead to 3 terminal states with different probabilities
...
# accumulated transition counts
Pk = np.zeros( (num_actions, num_next_states) )

# initialize the transition probability estimates with some intial samples, to avoid the zero transition probability crash in craam solver
for a in range(num_actions):
    for i in range(5):
        next_state = np.random.choice(num_next_states, 1, p=transitions[a])
        Pk[a,next_state] += 1
        Nk_[a] += 1
...
###
print(np.random.choice(3, 1, p=[0.6,0.2,0.2]))
