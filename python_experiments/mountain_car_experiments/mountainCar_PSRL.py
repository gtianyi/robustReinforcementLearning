    ...
    def train(self, q_init_ret):
        ...
        for run in range(self.num_runs):
            ...
            for episode in range(self.num_episodes):
                ...
                for s in self.all_states:
                    ...
                    for action in range(self.num_actions):
                        ...
                        for s_index, s_next in enumerate(next_states):
                            sampled_mdp.add_transition(cur_state, action, s_next, trp[s_index], get_reward(s_next, self.resolution, self.grid_x, self.grid_y)) 
                ...
                regret_psrl[run,episode] = abs(q_init_ret-expected_value_initial_state) #abs(cur_solution[0][0]-true_solution[0][0])
                ...
                for h in range(self.horizon):
                    ...
                    mp_rewards[index_to_single_index(next_state[0],next_state[1], self.resolution)] = reward
...
if __name__ == "__main__": 
    ...
    res = psrl_learn.train(0.0)
    print(res[2].valuefunction)
...
if __name__ == "__main__":
    ...
    for i in psrl_learn.grid_x:
        for j in psrl_learn.grid_y:
            p,v = obs_to_index((i,j), psrl_learn.env_low, psrl_learn.env_dx)
            position.append(i)
            velocity.append(j)
    ...
    def run(render=True, policy=None):
        ...
        for iter in range(max_iterations):
            ...
            if policy is None:
                ...
                state =  obs_to_index(obs, psrl_learn.env_low, psrl_learn.env_dx)
